...
def get_opts_ner():
    ...
    parser.add_argument(
        "--coarse_grain", action="store_true", default=False
    )  # fewnerd 粗粒度
    parser.add_argument(
        "--verbose_type", action="store_true", default=False
    )  # type 是否全写
    ...
    opts.type_file = os.path.join(
        opts.input_dir,
        os.path.join(opts.task, os.path.join(opts.dataset, opts.type_file)),
    )
    opts.test_file = os.path.join(
        opts.input_dir,
        os.path.join(opts.task, os.path.join(opts.dataset, opts.test_file)),
    )
    if opts.ICL:
    ...
    opts.result_file = os.path.join(
        opts.result_dir, os.path.join(opts.task, os.path.join(opts.dataset, res_file))
    )

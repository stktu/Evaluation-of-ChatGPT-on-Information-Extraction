        ...
        for ii in range(len(entity_list)):
            ...
            for jj in range(len(entity_list)):
                if ii != jj:
                    ...
                    if (start >= tar_start and end <= tar_end) or (
                        start <= tar_start and end >= tar_end
                    ):
...
def ner_nested_data_process(
    dataset, input_file, output_file, type_file, num_type, separator=" "
):
...
if __name__ == "__main__":
    ...
    ner_data_process(
        "ner/conll03/",
        "conll032_test_context.json",
        "ner_test.json",
        "conll032_types.json",
    )
    ner_data_process(
        "ner/conll03/",
        "conll032_train_dev_context.json",
        "ner_train.json",
        "conll032_types.json",
    )
    # print()

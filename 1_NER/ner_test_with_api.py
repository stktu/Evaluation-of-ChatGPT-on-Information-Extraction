...
from ner_report_metric import report_metric_by_file
...
def get_prompt_list(e_types):
    ...
    prompt = 'Considering {} types of named entities including {} and {}, recognize all named entities in the given sentence.\nAnswer in the format ["entity_type", "entity_name"] without any explanation. If no entity exists, then just answer "[]".'.format(
        len(e_types), ", ".join(e_types[:-1]), e_types[-1]
    )
...
def get_icl_cot_prompt_list(opts):
    ...
    if opts.ICL:
        prompt_icl_file = os.path.join(
            opts.input_dir, opts.task, opts.dataset, opts.icl_prompt
        )
        prompt_icl_list = json.load(open(prompt_icl_file, "r", encoding="utf-8"))
    elif opts.COT:
        prompt_cot_file = os.path.join(
            opts.input_dir, opts.task, opts.dataset, opts.cot_prompt
        )
...
def ner_get_prompt(opts, example, prompt_list, prompt_icl_list, prompt_cot_list):
    if opts.irrelevant:
        file_name = os.path.join(
            opts.input_dir, opts.task, opts.dataset, "train_no_entity.json"
        )
...
def get_best_prompt(opts, logger):
    ...
    f1_list = [
        report_metric_by_file(opts, file, logger, mode="strict", match="hard")
...
def ner_main(opts, bot, logger):
    ...
    with open(opts.test_file, "r", encoding="utf-8") as fr, open(
        ...
        e_types = [
            '"' + types["entities"][item]["short"] + '"' for item in types["entities"]
        ]
        if opts.verbose_type:
            e_types = [
                '"' + types["entities"][item]["verbose"] + '"'
    ...
    with open(opts.result_file, "a", encoding="utf-8") as fw:
        ...
        while i < len(selected_idx):
            ...
            prompt = ner_get_prompt(
                opts, example, prompt_list, prompt_icl_list, prompt_cot_list
            )
            print(example["seq"])
            ...
            result_list = []
            example.update({"NER": result_list, "prompt": prompt, "response": response})
            if opts.ICL or opts.COT:
    ...
    logger.write(
        "Times: {:.2f}s = {:.2f}m\n".format(
            end_time - start_time, (end_time - start_time) / 60.0
        )
    )
